...
def parse_heydealer_comps(json_data):
    ...
    for item in items:
        ...
        if "detail" in item and "auction" in item:
            ...
            def translate_heydealer_part(raw_p):
                if not raw_p: return ""
                EXACT_MAP = {
                    "door_front_driver": "운전석 앞도어", "door_front_passenger": "조수석 앞도어",
                    "door_rear_driver": "운전석 뒷도어", "door_rear_passenger": "조수석 뒷도어",
                    "door_front_left": "운전석 앞도어", "door_front_right": "조수석 앞도어",
                    "door_rear_left": "운전석 뒷도어", "door_rear_right": "조수석 뒷도어",
                    "door_driver_front": "운전석 앞도어", "door_passenger_front": "조수석 앞도어",
                    "door_driver_rear": "운전석 뒷도어", "door_passenger_rear": "조수석 뒷도어",
                    "fender_front_driver": "운전석 앞휀더", "fender_front_passenger": "조수석 앞휀더",
                    "fender_rear_driver": "운전석 쿼터패널", "fender_rear_passenger": "조수석 쿼터패널",
                    "fender_front_left": "운전석 앞휀더", "fender_front_right": "조수석 앞휀더",
                    "fender_rear_left": "운전석 쿼터패널", "fender_rear_right": "조수석 쿼터패널",
                    "quarter_panel_driver": "운전석 쿼터패널", "quarter_panel_passenger": "조수석 쿼터패널",
                    "quarter_panel_front_driver": "운전석 앞휀더", "quarter_panel_front_passenger": "조수석 앞휀더",
                    "quarter_panel_rear_driver": "운전석 쿼터패널", "quarter_panel_rear_passenger": "조수석 쿼터패널",
                    "front_bumper": "앞범퍼", "rear_bumper": "뒷범퍼",
                    "bumper_front": "앞범퍼", "bumper_rear": "뒷범퍼",
                    "hood": "보닛(후드)", "bonnet": "보닛(후드)", "trunk_lid": "트렁크리드", "trunk": "트렁크리드",
                    "tailgate": "테일게이트", "roof": "루프", "radiator_support": "라디에이터 서포트",
                    "front_panel": "프론트패널", "rear_panel": "리어패널",
                    "side_sill_driver": "운전석 사이드실", "side_sill_passenger": "조수석 사이드실",
                    "step_driver": "운전석 사이드스텝", "step_passenger": "조수석 사이드스텝",
                    "pillar_a_driver": "운전석 A필러", "pillar_a_passenger": "조수석 A필러",
                    "pillar_b_driver": "운전석 B필러", "pillar_b_passenger": "조수석 B필러",
                    "pillar_c_driver": "운전석 C필러", "pillar_c_passenger": "조수석 C필러",
                    "cross_member": "크로스멤버", "cross_member_front": "앞 크로스멤버", "cross_member_rear": "뒤 크로스멤버",
                    "inside_panel_driver": "운전석 인사이드패널", "inside_panel_passenger": "조수석 인사이드패널",
                    "inside_panel_front_driver": "운전석 앞 인사이드패널", "inside_panel_front_passenger": "조수석 앞 인사이드패널",
                    "inside_panel_rear_driver": "운전석 뒤 인사이드패널", "inside_panel_rear_passenger": "조수석 뒤 인사이드패널",
                    "side_member_driver": "운전석 사이드멤버", "side_member_passenger": "조수석 사이드멤버",
                    "side_member_front_driver": "운전석 앞 사이드멤버", "side_member_front_passenger": "조수석 앞 사이드멤버",
                    "side_member_rear_driver": "운전석 뒤 사이드멤버", "side_member_rear_passenger": "조수석 뒤 사이드멤버",
                    "wheel_house_driver": "운전석 휠하우스", "wheel_house_passenger": "조수석 휠하우스",
                    "wheel_house_front_driver": "운전석 앞 휠하우스", "wheel_house_front_passenger": "조수석 앞 휠하우스",
                    "wheel_house_rear_driver": "운전석 뒤 휠하우스", "wheel_house_rear_passenger": "조수석 뒤 휠하우스",
                    "dash_panel": "대쉬패널", "floor_panel": "플로어패널", "trunk_floor": "트렁크플로어",
                    "package_tray": "패키지트레이"
                }
                clean_p = str(raw_p).strip().lower()
                if clean_p in EXACT_MAP:
                    return EXACT_MAP[clean_p]
                tokens = clean_p.split('_')
                side = "운전석" if any(t in tokens for t in ["driver", "left", "lh"]) else "조수석" if any(t in tokens for t in ["passenger", "right", "rh"]) else ""
                pos = "앞" if "front" in tokens else "뒤" if any(t in tokens for t in ["rear", "back"]) else ""
                comp = ""
                if "inside" in tokens and "panel" in tokens: comp = "인사이드패널"
                elif "side" in tokens and "member" in tokens: comp = "사이드멤버"
                elif "cross" in tokens and "member" in tokens: comp = "크로스멤버"
                elif "wheel" in tokens and "house" in tokens: comp = "휠하우스"
                elif "side" in tokens and any(t in tokens for t in ["sill", "step"]): comp = "사이드실"
                elif "quarter" in tokens or ("fender" in tokens and pos == "뒤"): comp = "쿼터패널"
                elif "fender" in tokens: comp = "휀더"
                elif "door" in tokens: comp = "도어"
                elif "bumper" in tokens: comp = "범퍼"
                elif "pillar" in tokens:
                    p_type = [t for t in tokens if t in ["a", "b", "c"]]
                    comp = f"{p_type[0].upper()}필러" if p_type else "필러"
                elif "hood" in tokens or "bonnet" in tokens: comp = "보닛(후드)"
                elif "trunk" in tokens: comp = "트렁크"
                elif "roof" in tokens: comp = "루프"
                elif "radiator" in tokens or "support" in tokens: comp = "라디에이터 서포트"
                else: comp = " ".join([t for t in tokens if t not in ["driver", "passenger", "left", "right", "front", "rear", "lh", "rh"]])
                assembled = [p for p in [side, pos, comp] if p]
                return " ".join(assembled) if assembled else raw_p

            REPAIR_TYPE_MAP = {
                "exchange": "교환", "replace": "교환", "weld": "판금", "sheet_metal": "판금", "paint": "도색", "repair": "수리"
            }
            
            accident_repairs = d.get("accident_repairs", [])
            repair_items = []
            for r in accident_repairs:
                if isinstance(r, dict):
                    raw_p = r.get("part", "") or r.get("name", "") or ""
                    raw_t = r.get("repair", "") or r.get("type", "") or ""
                    p_name = translate_heydealer_part(raw_p)
                    t_name = REPAIR_TYPE_MAP.get(str(raw_t).strip().lower(), raw_t)
                    if p_name and t_name:
                        repair_items.append(f"{p_name}: {t_name}")
                    elif p_name:
                        repair_items.append(p_name)
                elif isinstance(r, str):
                    repair_items.append(translate_heydealer_part(r))
            
            tags = item.get("auction", {}).get("tags", [])
            tag_texts = [t.get("short_text") for t in tags if isinstance(t, dict) and t.get("short_text")]
            if tag_texts:
                accident_str = " ".join(tag_texts)
            else:
                has_accident = len(accident_repairs) > 0
                if has_accident:
                    accident_str = f"사고({', '.join(repair_items)})" if repair_items else "사고"
                else:
                    accident_str = "완전무사고"
            
            accident_detail = ", ".join(repair_items) if repair_items else ""
                
            car_spec = d.get("car_spec", {}) or {}
            spec_desc = car_spec.get("description", "")
            
            ended_at = item.get("auction", {}).get("ended_at_display")
            if ended_at:
                days_ago_str = ended_at.replace(" ", "")
            else:
                end_time = item.get("auction", {}).get("end_time") or d.get("created_at") or ""
                days_ago_str = "-"
                if end_time:
                    try:
                        import datetime
                        dt_str = end_time.replace("Z", "+00:00")
                        end_dt = datetime.datetime.fromisoformat(dt_str)
                        now_dt = datetime.datetime.now(datetime.timezone.utc)
                        days = (now_dt - end_dt).days
                        if days == 0: days_ago_str = "오늘"
                        elif days > 0: days_ago_str = f"{days}일전"
                    except Exception:
                        pass
            ...
            for line in spec_desc.split('\n'):
                ...
                if m:
                    ...
                    if opt and 'div' not in opt.lower() and not opt.startswith('<'):
                        options.append(opt)
            rows.append({
                "경과일": days_ago_str,
                "판매가_num": price,
        ...
        # 일반 시세 구조
            ...
            accident_str = "사고" if has_accident else "완전무사고"
            
            opts = item.get('options', item.get('tags', []))
            raw_options = [o.strip() for o in opts.split(',')] if isinstance(opts, str) else opts
            options = []
            if raw_options:
                import re
                for o in raw_options:
                    if isinstance(o, str):
                        clean_o = re.sub(r'<[^>]*>', '', o).strip()
                        if clean_o and 'div' not in clean_o.lower(): options.append(clean_o)
            
            end_time = item.get("created_at") or item.get("updated_at") or ""
            days_ago_str = "-"
            if end_time:
                try:
                    import datetime
...
class Scraper:
    ...
    def fetch_car_detail(session, c_id):
        ...
        acc = master.get("accdient") if master.get("accdient") is not None else master.get("accident")
        rep = master.get("simpleRepair")

        exch_cnt = 0
        sheet_cnt = 0

        # 1. 성능점검기록부(inspection) 데이터 우선 확인 (공인 원본 기록부)
        if i_resp.get("json"):
            ij = i_resp["json"]
            insp_parts = (ij.get("outers", []) or []) + (ij.get("inners", []) or [])
            if not insp_parts and "master" in ij:
                insp_parts = (ij["master"].get("outers", []) or []) + (ij["master"].get("inners", []) or [])
            for part in insp_parts:
                status_types = part.get("statusTypes", []) or []
                codes = [s.get("code", "") for s in status_types if isinstance(s, dict)]
                if "X" in codes:
                    exch_cnt += 1
                elif any(c in codes for c in ["W", "C", "A", "U", "T"]):
                    sheet_cnt += 1

        # 2. 성능점검기록부에 손상 부위가 없거나 누락된 경우에만 엔카 진단(diagnosis) 데이터 보강
        if (exch_cnt + sheet_cnt) == 0:
        ...
        def format_acc_counts(base, e_cnt, s_cnt):
            if e_cnt > 0 and s_cnt > 0:
                return f"{base} [교환:{e_cnt} / 판금:{s_cnt}]"
            elif e_cnt > 0:
                return f"{base} [교환:{e_cnt}]"
            elif s_cnt > 0:
                return f"{base} [판금:{s_cnt}]"
            return base

        if i_resp["status"] == 404:
            perf_date = "미검사/사진"
            accident_status = "기록부(사진)"
        elif acc is True:
            base_acc = "사고"
            accident_status = format_acc_counts(base_acc, exch_cnt, sheet_cnt)
        elif rep is True or exch_cnt > 0 or sheet_cnt > 0:
            if exch_cnt > 0 and sheet_cnt == 0:
                base_acc = "단순교환"
            elif exch_cnt == 0 and sheet_cnt > 0:
                base_acc = "단순판금"
            else:
                base_acc = "단순(교환/판금)" if (exch_cnt > 0 and sheet_cnt > 0) else "단순수리"
            accident_status = format_acc_counts(base_acc, exch_cnt, sheet_cnt)
        elif acc is False or rep is False:
            accident_status = "무사고"
...
st.subheader("🤖 헤이딜러 AI 매입 견적 (자동 수집)")

# 쿠키: .env에 저장된 값 자동 사용, 변경 필요 시에만 expander 열기
default_cookie = os.getenv("HEYDEALER_COOKIE", "")
with st.expander("🔑 쿠키 설정 (만료 시에만 열기)", expanded=False):
    st.caption("인증 오류 시 → [헤이딜러 딜러 로그인](https://dealer.heydealer.com) → F12 → Network → Cookie 복사 → 아래 붙여넣기")
    heydealer_cookie_input = st.text_input("Cookie 헤더 전체", value=default_cookie, type="password", label_visibility="collapsed")
if not heydealer_cookie_input:
    heydealer_cookie_input = default_cookie

col_url, col_btn = st.columns([4, 1])
with col_url:
    heydealer_url_input = st.text_input("헤이딜러 차량 URL 또는 ID", placeholder="https://dealer.heydealer.com/cars/xxxxxxxx/ 또는 ID", label_visibility="collapsed")
with col_btn:
    is_ai_running = st.session_state.get('ai_status') == "진행중"
    heydealer_btn_clicked = st.button("🚀 견적 산출", key="heydealer_btn", disabled=is_ai_running, use_container_width=True)

if heydealer_btn_clicked:
    if not heydealer_url_input.strip():
        st.warning("헤이딜러 차량 URL이나 ID를 입력해주세요.")
    elif not heydealer_cookie_input.strip():
        st.warning("⚠️ 쿠키가 없습니다. 위 '쿠키 설정'을 열어 입력해주세요.")
    else:
        try:
            ...
            with st.spinner("헤이딜러 서버에서 차량 정보를 가져오는 중입니다..."):
            ...
            # 🔥 헤이딜러 API 응답에 포함된 엔카 URL로 자동 엔카 스캔
            encar_auto_url = result.get('encar_url')
            if encar_auto_url:
                try:
                    with st.spinner("🔄 엔카 소매시세를 자동 수집 중입니다... (약 10초)"):
                        p_bar_auto = st.progress(0)
                        s_text_auto = st.empty()
                        new_scan_df, scan_msg = Scraper.run(encar_auto_url, "", p_bar_auto, s_text_auto)
                        if scan_msg == "success" and not new_scan_df.empty:
                            st.session_state.scan_data = pd.concat([st.session_state.scan_data, new_scan_df], ignore_index=True)
                            st.session_state.scan_data = st.session_state.scan_data.drop_duplicates(subset=['_carid'], keep='last').reset_index(drop=True)
                            ...
                            st.session_state.debug_success_msg += f" | ✅ 엔카 소매시세 자동 수집 완료 ({len(new_scan_df)}대)"
                        else:
                            st.session_state.debug_success_msg += f" | ⚠️ 엔카 자동 스캔 결과 없음 ({scan_msg})"
                        p_bar_auto.empty()
                        s_text_auto.empty()
                except Exception as encar_err:
                    st.session_state.debug_success_msg += f" | ⚠️ 엔카 자동 스캔 실패: {str(encar_err)[:50]}"
            else:
                st.session_state.debug_success_msg += " | ⚠️ 엔카 URL 미포함 (수동 스캔 필요)"
            ...
            try:
                ...
                if hd_year:
                    y_str = str(hd_year)
                    st.session_state[f"search_year_{reset_key}"] = y_str[-2:] if y_str.isdigit() and len(y_str) >= 2 else y_str
                if hd_mil:
            ...
            # 실시간 엔카 데이터 통계 사전 계산
            # 주의: filtered_df는 이 시점에서 아직 이전 데이터이므로, 방금 자동 스캔한 scan_data를 직접 사용
            market_data_str = ""
            encar_stats_df = st.session_state.get('scan_data', pd.DataFrame())
            if not encar_stats_df.empty:
                import pandas as pd
                df_temp = encar_stats_df.copy()
                ...
                try:
                    ...
                    if results:
                        ...
                        if not hd_comp_df.empty:
                            if target_year:
                                hd_same_year = hd_comp_df[hd_comp_df['연식'] == int(target_year)]
                                hd_prev = hd_comp_df[hd_comp_df['연식'] == int(target_year) - 1]
                                hd_next = hd_comp_df[hd_comp_df['연식'] == int(target_year) + 1]
                            else:
...
# 디버그 메시지 출력 제거 (UI 통일성 및 가독성 개선을 위함)
...
if status == "표시됨" and 'ai_estimate_result' in st.session_state:
    if 'hd_comp_df' in st.session_state and not st.session_state.hd_comp_df.empty:
        hd_col1, hd_col2 = st.columns([6.5, 3.5])
        
        with hd_col1:
            st.markdown("#### 📊 헤이딜러 매입시세 리스트")
            hd_display_df = st.session_state.hd_comp_df.copy()
            
            def summarize_hd_options(opt_list):
                if not opt_list or not isinstance(opt_list, list): return "-"
                return f"{len(opt_list)}개 옵션"
                
            hd_display_df["추가옵션_요약"] = hd_display_df["옵션리스트"].apply(summarize_hd_options)

            def format_hd_acc(acc_val):
                s = str(acc_val)
                if not s or s in ("-", "정보없음"): return s
                if "무사고" in s or "완무" in s: return f"🟢 {s}"
                elif "단순" in s: return f"🟡 {s}"
                elif "사고" in s: return f"🔴 {s}"
                return s
            hd_display_df["사고유무"] = hd_display_df["사고유무"].apply(format_hd_acc)
            
            try:
                hd_event = st.dataframe(
                    hd_display_df,
                    column_config={
                        "경과일": st.column_config.TextColumn("경과일", width=70),
                        "연식": st.column_config.TextColumn("연식", width=70),
                        "주행거리_num": st.column_config.NumberColumn("주행거리", format="%d km", width=90),
                        "판매가_num": st.column_config.NumberColumn("판매가", format="%d 만원", width=90),
                        "사고유무": st.column_config.TextColumn("사고유무", width=110),
                        "추가옵션_요약": st.column_config.TextColumn("추가옵션"),
                        "옵션리스트": None,
                    },
                    column_order=["경과일", "연식", "주행거리_num", "판매가_num", "사고유무", "추가옵션_요약"],
                ...
                hd_event = st.dataframe(hd_display_df, use_container_width=False, hide_index=True)
                
        with hd_col2:
            st.markdown("#### 🚘 상세 스펙 및 옵션 (헤이딜러)")
            hd_selected_rows = hd_event.selection.rows if hasattr(hd_event, "selection") else []
            if hd_selected_rows:
                sel_idx = hd_selected_rows[0]
                row = st.session_state.hd_comp_df.iloc[sel_idx]
                
                base_acc = str(row['사고유무'])
                for t in ["1인소유", "대여", "용도이력", "법인"]:
                    base_acc = base_acc.replace(t, "").strip()
                base_acc = " ".join(base_acc.split())
                if '무사고' in base_acc or '완무' in base_acc or base_acc == '완전무사고':
                ...
                acc_html = f"<div style='margin-bottom: 5px;'><span style='color: #aaa; margin-right:8px;'>사고내역:</span> <span style='{hd_acc_style} padding:2px 10px; border-radius:12px; font-weight: bold; font-size:0.9em; display:inline-block;'>{hd_acc_icon} {base_acc}</span></div>"
                if acc_detail:
                    issue_items = []
                    for item in [p.strip() for p in acc_detail.split(",") if p.strip()]:
                        bg_color = "#ff4d4d" if "교환" in item else "#ffdd57"
                        text_color = "#fff" if "교환" in item else "#333"
                        issue_items.append(f"<span style='background:{bg_color}; color:{text_color}; padding:2px 8px; border-radius:12px; font-size:0.85em; margin-right:5px; margin-bottom:5px; display:inline-block; box-shadow:0 1px 2px rgba(0,0,0,0.1);'>{item}</span>")
                    acc_html += f"<div style='margin-top: 10px; padding-top: 10px; border-top: 1px dashed #444;'><div style='color: #aaa; font-size: 0.85em; margin-bottom: 5px; font-weight:bold;'>🛠️ 수리 부위</div><div style='margin-top:5px; display:flex; flex-wrap:wrap;'>{' '.join(issue_items)}</div></div>"
                    
                option_style = "background-color:#2a2a35; border:1px solid #444; padding:4px 8px; border-radius:4px; font-size:0.85em; color:#ddd; display:inline-flex; align-items:center; line-height:1.2;"
                
                raw_opts = row['옵션리스트'] if isinstance(row['옵션리스트'], list) else []
                ...
                if not options_badges:
                    options_badges = '<span style="color:#999;">등록된 옵션 없음</span>'
                
                days_str = row.get('경과일', '')
                days_html = f"<div style='text-align:right; margin-bottom:10px;'><span style='color:#888; font-size:0.9em;'>{days_str}</span></div>" if days_str else ""
                
                card_html = (
                    f'<div class="spec-card" style="background-color:#1e1e24; padding:20px; border-radius:8px; border:1px solid #333;">'
                    f'<h3 style="margin-top:0; margin-bottom:20px; color:#fff;">선택한 차량 정보</h3>'
                    f'<div style="display:flex; justify-content:space-between; margin-bottom:10px;"><span style="color:#aaa;">연식</span><strong style="color:#fff;">{row["연식"]}</strong></div>'
                    f'<div style="display:flex; justify-content:space-between; margin-bottom:10px;"><span style="color:#aaa;">주행거리</span><strong style="color:#fff;">{row["주행거리_num"]:,} km</strong></div>'
                    f'<div style="display:flex; justify-content:space-between; margin-bottom:5px;"><span style="color:#aaa;">판매가(낙찰가)</span><strong style="color:#ff4b4b; font-size:1.2em;">{row["판매가_num"]:,} 만원</strong></div>'
                    f'{days_html}'
                    f'<hr style="border:0; border-top:1px dashed #444; margin:15px 0;">'
                    f'<div style="margin-bottom:15px;"><span style="color:#aaa;">상태 정보</span><div style="margin-top:8px;">{acc_html}</div></div>'
                    f'<div><span style="color:#aaa;">주요 옵션</span><div style="margin-top:10px; display:flex; flex-wrap:wrap; gap:5px; align-items:flex-start;">{options_badges}</div></div>'
                    f'</div>'
                )
                st.markdown(card_html, unsafe_allow_html=True)
            else:
                st.info("👈 리스트에서 차량을 선택하면 상세 정보를 볼 수 있습니다.")
...
with tab_main:
    ...
    <div style='display: flex; gap: 20px; margin-top: 15px;'>
        <div class='metric-card' style='flex: 1;'>
            ...
            <div class='metric-content'><h4>총 매물 수</h4><h2>{total_count} 대</h2></div>
        </div>
        <div class='metric-card' style='flex: 1;'>
            <div class='metric-icon'>⬇️</div>
            <div class='metric-content'><h4>최저가</h4><h2 style='color: #0066cc;'>{min_price:,} 만원 <span style='font-size: 0.6em'>⬇️</span></h2></div>
        </div>
        <div class='metric-card' style='flex: 1;'>
            <div class='metric-icon'>⬆️</div>
            <div class='metric-content'><h4>최고가</h4><h2 style='color: #cc0000;'>{max_price:,} 만원 <span style='font-size: 0.6em'>⬆️</span></h2></div>
        </div>
        <div class='metric-card' style='flex: 1;'>
            <div class='metric-icon'>📊</div>
            <div class='metric-content'><h4>평균가</h4><h2>{avg_price:,} 만원</h2></div>
        </div>
    </div>
    """
    ...
    if not filtered_df.empty:
        main_col1, main_col2 = st.columns([6.5, 3.5])

        with main_col1:
            st.markdown("#### 📊 시세 스캔 리스트")
            ...
            display_df["사고유무"] = display_df["사고유무"].apply(format_display_acc)
    
            def make_link_name(row):
                link = row.get('링크')
                if pd.notna(link) and str(link).startswith('http'):
                    name = row.get('차량명', '상세보기')
                    return f"{link}#_name={name}"
                return link
            ...
            try:
                event = st.dataframe(
                    ...
                    column_config={
                        ...
                        "차량명_링크": st.column_config.LinkColumn("차량명", width=80, display_text=r"#_name=(.*)"),
                        "세부모델": st.column_config.TextColumn("세부모델", width=180),
                        "연식": st.column_config.TextColumn("연식", width=60),
                        "주행거리": st.column_config.NumberColumn("주행거리", width=75, format="%d km"),
                        "판매가": st.column_config.NumberColumn("판매가", width=70, format="%d 만원"),
                        "재고": st.column_config.TextColumn("재고", width=50),
                        "사고유무": st.column_config.TextColumn("사고유무", width=125),
                        "외장컬러": st.column_config.TextColumn("외장컬러", width=60),
                        "추가옵션_요약": st.column_config.TextColumn("추가옵션", width=140),
                        "링크": None,
                        "_carid": None
                    },
                    column_order=["성능일", "차량명_링크", "세부모델", "연식", "주행거리", "판매가", "재고", "사고유무", "외장컬러", "추가옵션_요약", "제조사"],
                    use_container_width=False,
                    height=750,  
                    hide_index=True,
                    on_select="rerun",
                ...
                event = st.dataframe(display_df, use_container_width=False, height=750, hide_index=True)
            ...
            if selected_rows:
                ...
                def get_damage_info(carid):
                    ...
                    try:
                        ...
                        # 1. 성능점검기록부(inspection) 데이터 확인 (공인 원본 기록부)
                ...
                PART_COORDS_OUTER = [
                    ([("후드", "보닛")], [], 55, 20,  110, 55, "후드", "후드"),
                    ([("프론트", "앞"), ("휀더", "펜더"), ("좌", "운전석")], ["뒤", "리어", "쿼터"], 15, 25,  35, 65, "F휀", "프론트 휀더(좌)"),
                    ([("프론트", "앞"), ("휀더", "펜더"), ("우", "조수석")], ["뒤", "리어", "쿼터"], 170, 25,  35, 65, "F휀", "프론트 휀더(우)"),
                    ([("프론트", "앞"), ("도어", "문"), ("좌", "운전석")],   ["뒤", "리어", "백"], 10, 100, 40, 60, "F도", "프론트 도어(좌)"),
                    ([("프론트", "앞"), ("도어", "문"), ("우", "조수석")],   ["뒤", "리어", "백"], 170, 100, 40, 60, "F도", "프론트 도어(우)"),
                    ([("리어", "뒤"), ("도어", "문"), ("좌", "운전석")],     ["앞", "프론트"], 10, 165, 40, 60, "R도", "리어 도어(좌)"),
                    ([("리어", "뒤"), ("도어", "문"), ("우", "조수석")],     ["앞", "프론트"], 170, 165, 40, 60, "R도", "리어 도어(우)"),
                    ([("쿼터", "뒤휀더", "리어휀더"), ("좌", "운전석")],      ["앞", "프론트"], 10, 230, 40, 60, "쿼터", "쿼터 패널(리어펜더)(좌)"),
                    ([("쿼터", "뒤휀더", "리어휀더"), ("우", "조수석")],      ["앞", "프론트"], 170, 230, 40, 60, "쿼터", "쿼터 패널(리어펜더)(우)"),
                    ([("루프", "지붕")],                                     [], 55, 80,  110, 145, "루프", "루프"),
                    ([("트렁크", "테일게이트", "백도어")],                    ["플로어"], 55, 230, 110, 60, "TR", "트렁크리드"),
                ]

                PART_COORDS_INNER = [
                    ([("사이드멤버", "사이드 멤버"), ("프론트", "앞"), ("좌", "운전석")], ["뒤", "리어"], 10, 30,  40, 55, "F멤", "프론트 사이드멤버(좌)"),
                    ([("사이드멤버", "사이드 멤버"), ("프론트", "앞"), ("우", "조수석")], ["뒤", "리어"], 170, 30,  40, 55, "F멤", "프론트 사이드멤버(우)"),
                    ([("크로스멤버", "크로스 멤버")],                               [], 55, 30,  110, 30, "크로스", "크로스멤버"),
                    ([("라디에이터", "서포트", "R.S")],                           [], 55, 65,  110, 30, "R.S", "라디에이터 서포트"),
                    ([("인사이드",), ("좌", "운전석")],                            ["사이드멤버"], 10, 100, 40, 120, "I패", "인사이드 패널(좌)"),
                    ([("인사이드",), ("우", "조수석")],                            ["사이드멤버"], 170, 100, 40, 120, "I패", "인사이드 패널(우)"),
                    ([("사이드멤버", "사이드 멤버"), ("리어", "뒤"), ("좌", "운전석")], ["앞", "프론트"], 10, 230, 40, 60, "R멤", "리어 사이드멤버(좌)"),
                    ([("사이드멤버", "사이드 멤버"), ("리어", "뒤"), ("우", "조수석")], ["앞", "프론트"], 170, 230, 40, 60, "R멤", "리어 사이드멤버(우)"),
                    ([("플로어", "트렁크플로어")],                                  [], 55, 230, 110, 60, "T플", "트렁크 플로어"),
                    ([("리어패널", "리어 패널", "뒤패널", "뒤 패널", "백패널")],    ["인사이드", "사이드"], 55, 290, 110, 20, "R패", "리어 패널"),
                ]
                ...
                def find_status(damage_data, synonyms_list, exclude_words=None):
                    ...
                    for name, status in damage_data.items():
                        ...
                        for group in synonyms_list:
                            if not any(syn in name for syn in group):
                                match = False
                                break
                        if match:
                            return status
                ...
                def render_panel_svg(damage_data, coords, panel_title):
                    shapes = ""
                    for synonyms_list, exclude_words, x, y, w, h, short_label, full_label in coords:
                        status = find_status(damage_data, synonyms_list, exclude_words)
                        color = STATUS_COLOR.get(status, "#e2e8f0")
                        tx, ty = x + w / 2, y + h / 2
                        shapes += f"""<g>
    <title>{full_label} : {status}</title>
    <rect x="{x}" y="{y}" width="{w}" height="{h}" rx="6" fill="{color}" stroke="#64748b" stroke-width="1.2"/>
    <text x="{tx}" y="{ty}" text-anchor="middle" dominant-baseline="middle" font-size="10" fill="#333">{short_label}</text>
    </g>"""

                    return f"""
    <div style="text-align:center;">
      <div style="font-weight:bold; margin-bottom:6px; color:#555;">{panel_title}</div>
      <svg viewBox="0 0 220 340" style="width:100%; max-width:250px;">
        <rect x="15" y="10" width="190" height="320" rx="30" fill="#f8fafc" stroke="#94a3b8" stroke-width="1"/>
        {shapes}
      </svg>
    </div>
    """
                ...
                def render_car_diagram(damage_data):
                    ...
                    svg = f"""
    <div style='background-color: white; color: #333; border-radius: 10px; padding: 20px; border: 1px solid #ddd; box-shadow: 0 4px 6px rgba(0,0,0,0.05);'>
    <h4 style='color: #555; text-align:center; margin-bottom: 15px;'>🚨 성능기록부 사고 부위 오버레이</h4>
    ...
    <span style='margin-right: 10px;'><span style='color: #ffdd57;'>■</span> 판금/부식/흠집/요철/손상</span>
    <span><span style='color: #e2e8f0;'>■</span> 정상</span>
    </div>
    </div>
    """
                    return svg
                ...
                for opt in opt_items[:8]:
                    opt_html += f"<div style='background:#f0f2f6; color:#333; padding:5px 10px; border-radius:15px; font-size:0.8em; margin:3px; display:inline-block;'>✓ {opt}</div>"
                if len(opt_items) > 8:
                    opt_html += f"<div style='background:#f0f2f6; color:#333; padding:5px 10px; border-radius:15px; font-size:0.8em; margin:3px; display:inline-block;'>+{len(opt_items)-8} 더보기</div>"
                ...
                if carid:
                    ...
                    if issues:
                        ...
                        for k, v in issues.items():
                            bg_color = "#ff4d4d" if v == "교환" else "#ffdd57"
                            text_color = "#fff" if v == "교환" else "#333"
                            issue_items.append(f"<span style='background:{bg_color}; color:{text_color}; padding:2px 8px; border-radius:12px; font-size:0.85em; margin-right:5px; margin-bottom:5px; display:inline-block; box-shadow:0 1px 2px rgba(0,0,0,0.1);'>{k}: {v}</span>")
                        damage_summary_html = f"<div style='margin-top: 10px; padding-top: 10px; border-top: 1px dashed #ddd;'><div style='color: #555; font-size: 0.85em; margin-bottom: 5px; font-weight:bold;'>🛠️ 상세 손상 부위</div>{' '.join(issue_items)}</div>"
                    elif damage_data:
                        damage_summary_html = f"<div style='margin-top: 10px; padding-top: 10px; border-top: 1px dashed #ddd;'><div style='color: #38a169; font-size: 0.9em; font-weight:bold;'>✨ 특이사항 없음 (모든 부위 정상)</div></div>"

                acc_val = str(row['사고유무'])
                # 실시간 damage_data 손상 이력이 확인되면 가장 정확한 부위 카운트로 자동 보정
                if damage_data:
                    real_e = sum(1 for v in damage_data.values() if v == "교환")
                    real_s = sum(1 for v in damage_data.values() if v == "판금")
                    if "사고" in acc_val and "무사고" not in acc_val:
                        base_tag = "사고"
                    elif real_e > 0 and real_s == 0:
                        base_tag = "단순교환"
                    elif real_e == 0 and real_s > 0:
                        base_tag = "단순판금"
                    elif real_e > 0 and real_s > 0:
                        ...
                        base_tag = "무사고" if (real_e == 0 and real_s == 0 and ("무사고" in acc_val or "완무" in acc_val)) else "단순수리"

                    if real_e > 0 and real_s > 0:
                        acc_val = f"{base_tag} [교환:{real_e} / 판금:{real_s}]"
                    elif real_e > 0:
                        acc_val = f"{base_tag} [교환:{real_e}]"
                    elif real_s > 0:
                        acc_val = f"{base_tag} [판금:{real_s}]"
                    else:
                        acc_val = base_tag
                else:
                    # damage_data가 없더라도 기존 잔여 [판금:0], [교환:0] 정제
                    s = acc_val
                    s = re.sub(r'\s*/\s*판금:0', '', s)
                    s = re.sub(r'교환:0\s*/\s*', '', s)
                    s = re.sub(r'\[교환:0\]', '', s)
                ...
                if "무사고" in acc_val or "완무" in acc_val:
                    acc_badge_style = "background:#ecfdf5; color:#15803d; border:1px solid #86efac;"
                    acc_icon = "🟢"
                elif "단순" in acc_val or "판금" in acc_val or "교환" in acc_val:
                    acc_badge_style = "background:#fefce8; color:#b45309; border:1px solid #fde047;"
                    acc_icon = "🟡"
                elif "사고" in acc_val:
                    acc_badge_style = "background:#fef2f2; color:#b91c1c; border:1px solid #fca5a5;"
                    acc_icon = "🔴"
                else:
                    acc_badge_style = "background:#f3f4f6; color:#4b5563; border:1px solid #e5e7eb;"
                    acc_icon = "⚪"

                detail_html = f"""
<div style='background-color: white; color: #333; border-radius: 10px; padding: 20px; border: 1px solid #ddd; box-shadow: 0 4px 6px rgba(0,0,0,0.05);'>
<div style='display: flex; align-items: center; border-bottom: 2px solid #f0f2f6; padding-bottom: 10px; margin-bottom: 15px;'>
<div style='font-size: 2em; margin-right: 15px;'>🚙</div>
<div>
<h3 style='margin: 0; color: #333;'>{row['차량명']}</h3>
</div>
</div>
<h4 style='color: #555; margin-bottom: 10px;'>기본 정보</h4>
<table style='width: 100%; border-collapse: collapse; margin-bottom: 20px;'>
<tr><td style='padding: 5px 0; color: #666; width: 40%; border-bottom: 1px solid #f0f2f6;'>모델</td><td style='padding: 5px 0; font-weight: bold; border-bottom: 1px solid #f0f2f6;'>{row['세부모델']}</td></tr>
<tr><td style='padding: 5px 0; color: #666; border-bottom: 1px solid #f0f2f6;'>연식</td><td style='padding: 5px 0; font-weight: bold; border-bottom: 1px solid #f0f2f6;'>{row['연식']}</td></tr>
<tr><td style='padding: 5px 0; color: #666; border-bottom: 1px solid #f0f2f6;'>주행거리</td><td style='padding: 5px 0; font-weight: bold; border-bottom: 1px solid #f0f2f6;'>{format_num(row['주행거리'])} km</td></tr>
<tr><td style='padding: 5px 0; color: #666; border-bottom: 1px solid #f0f2f6;'>색상</td><td style='padding: 5px 0; font-weight: bold; border-bottom: 1px solid #f0f2f6;'>{row.get('외장컬러', '-')}</td></tr>
<tr><td style='padding: 5px 0; color: #666;'>판매가</td><td style='padding: 5px 0; font-weight: bold; color: #e53e3e;'>{format_num(row['판매가'])} 만원</td></tr>
</table>
<h4 style='color: #555; margin-bottom: 10px;'>상태 정보</h4>
<div style='background-color: #f8f9fa; color: #333; padding: 15px; border-radius: 8px; margin-bottom: 20px;'>
<div style='margin-bottom: 8px;'><span style='color: #666; margin-right: 8px;'>사고내역:</span> <span style='{acc_badge_style} padding: 3px 10px; border-radius: 12px; font-weight: bold; font-size: 0.9em; display: inline-block;'>{acc_icon} {acc_val}</span></div>
<div style='margin-bottom: 5px;'><span style='color: #666;'>성능점검일:</span> <span>{row['성능일']}</span></div>
{damage_summary_html}
</div>
<h4 style='color: #555; margin-bottom: 10px;'>핵심 옵션</h4>
<div style='display: flex; flex-wrap: wrap;'>
{opt_html if opt_html else "<span style='color: #999;'>옵션 정보 없음</span>"}
</div>
</div>
"""
                ...
                if carid:
                    if st.toggle("🔍 성능기록부 도면 전체 보기", key=f"btn_poc_{carid}"):
                        svg_html = render_car_diagram(damage_data)
        ...
        if not valid_prices.empty:
            ...
            if not chart_df.empty:
                ...
                def get_color(acc):
                    acc_str = str(acc)
                    if "무사고" in acc_str or "완무" in acc_str: return '#22C55E' 
                    elif "단순" in acc_str or "판금" in acc_str or "교환" in acc_str: return '#EAB308' 
                    elif "사고" in acc_str: return '#EF4444' 
                    else: return '#9CA3AF'
                ...
                hover_text = [
                    f"출처: {r['상태']}<br>{r['연식']}년식 · 성능일 {r['성능일']}<br>{r['사고유무']}"
